...
export function calculateRentPotential(
  ...
  const timeToReachMarket = potential > 0 
    ? Math.ceil(Math.log(marketRent / currentRent) / Math.log(1 + yearlyIncreasePercent / 100))
    : 0;
  
  let recommendation: string;
  if (potentialPercent > 20) {
    recommendation = 'Erhebliches Mieterhöhungspotenzial. Eine schrittweise Anpassung innerhalb der gesetzlichen Grenzen ist empfehlenswert.';
  } else if (potentialPercent > 10) {
    recommendation = 'Moderates Mieterhöhungspotenzial vorhanden. Eine Mietanpassung könnte geprüft werden.';
  } else if (potentialPercent > 0) {
    recommendation = 'Geringes Mieterhöhungspotenzial. Die aktuelle Miete liegt nahe am Marktniveau.';
  } else {
    recommendation = 'Die aktuelle Miete liegt über dem Marktniveau. Bei Neuvermietungen ist Vorsicht geboten.';
  }

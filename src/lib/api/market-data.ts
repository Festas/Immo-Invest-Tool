...
export async function fetchMarketData(postalCode: string): Promise<MarketPriceData> {
  ...
  const regionMatch = Object.values(CITY_MARKET_DATA).find((data) =>
    data.postalCode.startsWith(regionPrefix)
  );
...
export async function fetchPricesByType(postalCode: string): Promise<PropertyTypePrice[]> {
  ...
  return [
    { type: "WOHNUNG", pricePerSqm: basePpsqm, typicalSize: { min: 40, max: 120 } },
    {
      ...
      pricePerSqm: Math.round(basePpsqm * 1.1),
      typicalSize: { min: 100, max: 250 },
    },
    {
      type: "MEHRFAMILIENHAUS",
      pricePerSqm: Math.round(basePpsqm * 0.85),
      typicalSize: { min: 200, max: 600 },
    },
    {
      type: "GEWERBE",
      pricePerSqm: Math.round(basePpsqm * 1.2),
      typicalSize: { min: 50, max: 500 },
    },
  ];
}

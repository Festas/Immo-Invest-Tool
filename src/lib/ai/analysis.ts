...
function identifyRisks(
  ...
  if (output.cashflow.monthlyCashflowAfterTax < -300) {
    risks.push({
      ...
      mitigation:
        "Prüfen Sie Mieterhöhungspotenzial, niedrigere Finanzierungskosten oder höheres Eigenkapital.",
    });
...
export function generateInsights(input: PropertyInput, output: PropertyOutput): AIInsight[] {
  ...
  if (input.interestRate > 3.5) {
    insights.push({
      ...
      content:
        "Bei aktuellem Marktniveau könnten günstigere Konditionen möglich sein. Ein Vergleich verschiedener Finanzierungsanbieter wird empfohlen.",
      confidence: "MEDIUM",
    });
  }
  ...
  if (equityRatio > 40) {
    insights.push({
      ...
      content:
        "Mit dem hohen Eigenkapitalanteil könnte alternativ ein weiteres Objekt finanziert werden, um die Rendite zu hebeln.",
      confidence: "MEDIUM",
    });
  }
...
export function answerInvestmentQuestion(
  ...
  if (q.includes("cashflow") || q.includes("monatlich")) {
    return (
      `Der monatliche Cashflow nach Steuern beträgt ${context.output.cashflow.monthlyCashflowAfterTax.toFixed(0)}€. ` +
  ...
  if (q.includes("rendite") || q.includes("yield")) {
    return (
      `Die Bruttomietrendite liegt bei ${context.output.yields.grossRentalYield.toFixed(2)}%, ` +
  ...
  if (q.includes("finanzierung") || q.includes("kredit")) {
    return (
      `Die Finanzierung läuft über ${context.output.financing.loanAmount.toFixed(0)}€ bei ` +
  ...
  if (q.includes("steuer") || q.includes("afa")) {
    return (
      `Jährliche Steuerersparnis durch AfA: ${context.output.tax.afaAmount.toFixed(0)}€. ` +
